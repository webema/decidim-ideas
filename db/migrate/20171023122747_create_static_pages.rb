...
class CreateStaticPages < ActiveRecord::Migration[5.1]
  ...
  def change
    Organization.find_each do |organization|
      StaticPage.find_or_create_by!(slug: "ideas") do |page|
        page.decidim_organization_id = organization.id
        page.title = localized_attribute(organization, "ideas", :title)
        page.content = localized_attribute(organization, "ideas", :content)
      end
    end
  end
