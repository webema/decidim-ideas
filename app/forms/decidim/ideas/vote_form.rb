...
#       attribute :idea, Decidim::Idea
#       attribute :signer, Decidim::User

#       validates :idea, :signer, presence: true
...
#       # Public: The hash to uniquely identify an idea vote. It uses the
#       # idea scope as a default.
#       #
#       # Returns a String.
#       def hash_id
#         return unless idea && (document_number || signer)
...
#       # the idea allows also voting on child scopes, not only the main scope.
#       #
#       # Instead of just listing the children of the main scope, we just want to select the ones that
#       # have been added to the IdeaType with its voting settings.
#       #
#       def authorized_scopes
#         idea.votable_idea_type_scopes.select do |idea_type_scope|
#           idea_type_scope.global_scope? ||
#             idea_type_scope.scope == user_authorized_scope ||
#             idea_type_scope.scope.ancestor_of?(user_authorized_scope)
#         end.flat_map(&:scope)
#       end
...
#       # scope to sign an idea with a parent scope.
#       #
...
#       # in district, neighbourhoods or city ideas.
#       #
...
#         authorized_scope_candidates = [idea.scope]
#         authorized_scope_candidates += if idea.scope.present?
#                                          idea.scope.descendants
#                                        else
#                                          idea.organization.scopes
#                                        end
...
#       # Private: Whether the personal data given when signing the idea should
#       # be stored together with the vote or not.
#       #
#       # Returns a Boolean.
#       def required_personal_data?
#         @required_personal_data ||= idea&.type&.collect_user_extra_fields?
#       end
...
#       # ideas with someone elses identity.
#       def document_number_authorized?
#         return if idea.document_number_authorization_handler.blank?
...
#       def already_voted?
#         errors.add(:document_number, :taken) if idea.votes.exists?(hash_id: hash_id, scope: scope)
#       end
...
#       # Private: Finds an authorization for the user signing the idea and
...
#       # Returns a String.
#       def handler_name
#         idea.document_number_authorization_handler
#       end
