    ...
    module Admin
      module Filterable
        ...
        included do
          ...
          def base_query
            collection.joins(:scoped_type).left_joins(:area).joins("JOIN decidim_users ON decidim_users.id = decidim_ideas.decidim_author_id")
          end
          ...
          def filters_with_values
            {
              state_eq: Idea.states.keys,
              type_id_eq: IdeasType.where(organization: current_organization).pluck(:id),
          ...
          def translated_type_id_eq(id)
            translated_attribute(Decidim::IdeasType.find_by(id: id).title[I18n.locale.to_s])
          end
