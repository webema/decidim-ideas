  ...
  module Ideas
    module Admin
      describe IdeaForm do
        ...
        let(:other_scope) { create(:ideas_type_scope, type: ideas_type) }
        let(:state) { "published" }
        ...
        describe "#map_model" do
          ...
          context "when there are offline votes" do
            context "with a specific scope" do
              context "when there are no votes" do
                it "returns the correct offline_votes" do
                  ...
                  expected = { idea.scope.id.to_s.to_sym => [0, { "ca" => scope_name, "en" => scope_name, "es" => scope_name }] }
                  expect(subject.offline_votes).to eq expected
                end
              end

              context "when there are votes" do
                before { idea.update!(offline_votes: { "total" => "100", idea.scope.id.to_s => "100" }) }

                it "returns the correct offline_votes" do
                  scope_name = scope.scope_name["en"]
                  expected = { idea.scope.id.to_s.to_sym => ["100", { "ca" => scope_name, "en" => scope_name, "es" => scope_name }] }
                  expect(subject.offline_votes).to eq expected
                end
              end
            end
            ...
            context "with the global scope" do
              let(:scope) { create(:ideas_type_scope, scope: nil) }
              ...
              context "when there are votes" do
                before { idea.update!(offline_votes: { "total" => "100", "global" => "100" }) }
        ...
        describe "#signature_type_updatable?" do
          subject { described_class.from_model(idea).with_context(context).signature_type_updatable? }
        ...
        describe "#scoped_type_id" do
          context "when created from attributes" do
            ...
            context "when type_id and decidim_scope_id from idea are provided" do
              it { is_expected.to eq(idea.scoped_type.id) }
            end
          ...
          context "when created from model" do
            subject { described_class.from_model(idea).with_context(context).scoped_type_id }
