...
module Decidim
  module Ideas
    describe IdeaForm do
      ...
      context "when idea type enables custom signature end date" do
        let(:ideas_type) { create(:ideas_type, :custom_signature_end_date_enabled, organization: organization) }
      ...
      context "when idea type enables area" do
        let(:ideas_type) { create(:ideas_type, :area_enabled, organization: organization) }
      ...
      describe "#area_updatable?" do
        ...
        context "when idea is created" do
          subject { described_class.from_model(idea).with_context(context).area_updatable? }

          let(:idea) { create(:idea, organization: organization, state: "created", scoped_type: scope) }
        ...
        context "when user is not admin and idea is not created the area is not updatable" do
          subject { described_class.from_model(idea).with_context(context).area_updatable? }
