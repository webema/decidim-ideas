    ...
    describe VoteIdea do
      ...
      describe "User votes idea" do
        ...
        it "notifies the creation" do
          follower = create(:user, organization: idea.organization)
          create(:follow, followable: idea.author, user: follower)
        ...
        it "sends notification with email" do
          ...
          expect(last_email_body).to include("has endorsed the following idea")
        end
        ...
        context "when a new milestone is completed" do
          ...
          before do
            create(:idea_user_vote, idea: idea)
            create(:idea_user_vote, idea: idea)
          end
        ...
        context "when idea type requires extra user fields" do
          let(:idea) do
            create(
              :idea,
          ...
          context "when another signature exists with the same hash_id" do
            before do
              create(:idea_user_vote, idea: idea, hash_id: form_with_personal_data.hash_id)
            end
          ...
          context "when idea type has document number authorization handler" do
            let(:handler_name) { "dummy_authorization_handler" }
            let(:unique_id) { "test_digest" }
            let(:metadata) do
              {
                test: "dummy",
                scope_id: idea.scoped_type.scope.id
              }
            end
            ...
            before do
              ...
              idea.type.update(document_number_authorization_handler: handler_name)
            end
