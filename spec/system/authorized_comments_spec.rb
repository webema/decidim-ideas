...
describe "Authorized comments", type: :system do
  ...
  let(:commentable) { create(:idea, :published, author: author, scoped_type: scoped_type, organization: organization) }
  ...
  context "when the idea has no restriction on commenting" do
    before do
      visit resource_path
    end
  ...
  context "when the idea has restrictions on commenting" do
